    ...
    public class Rule4
    {
        ...
        public void checkCandle9()
        {
            if (currentCandle == 9)
            {
                ...
                if (block0[6].CompareTo(block0[9]) == 0)
                {
                    lostChain = 0;
                }
                else
        ...
        public void checkCandle13()
        {
            if (currentCandle == 13)
            {
                ...
                if ((block0[6].CompareTo(block0[9]) != 0) &&
                    (block0[6].CompareTo(block0[11]) != 0)
                    )
                {
                    if ((block0[6].CompareTo("blue") == 0))
                    {
        ...
        public void checkCandle15()
        {
            if (currentCandle == 15)
            {
                ...
                if ((block0[6].CompareTo(block0[9]) != 0) &&
                  (block0[6].CompareTo(block0[11]) != 0) &&
                  (block0[6].CompareTo(block0[15]) == 0)
                  )
                {
                    if ((block0[6].CompareTo("blue") == 0))
                    {
        ...
        public void checkCandle17()
        {
            if (currentCandle == 17)
            {
                if ((block0[6].CompareTo(block0[9]) != 0) &&
                  (block0[6].CompareTo(block0[11]) != 0) &&
                  (block0[6].CompareTo(block0[15]) == 0) &&
                  (block0[6].CompareTo(block0[17]) == 0)
                  )
                {
                    updateResultCommand();
                    lostChain++;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        numberTmp = (string)js.ExecuteScript("var money = document.querySelector('.buttonBalance.d-flex.align-items-center').querySelector('.d-flex.flex-column.mr-lg-2.mr-2').querySelector('.d-flex.align-items-center').getElementsByTagName('span')[0].textContent; var lengthX = money.length; var moneytmp2= money.slice(1,lengthX); var moneyTmp = parseFloat(moneytmp2.replace(/,/g,'')); return moneyTmp.toString();");
                        currentMoney = float.Parse(numberTmp);
                        chromeDriver.ExecuteScript("var popupwin = window.open('https://api.telegram.org/bot1477417480:AAHtpkU2nrwMiTC_zl-rThkFTLEOE2xbEEA/sendMessage?chat_id=-1001398145297' + '&text=[BOT_RESULT] SET COMMAND - PP Căn theo nến 6:    %0A Đã thua Block này %0A Số Tiền hiện tại : " + currentMoney + "  ', 'anyname', 'width=10,height=1,left=5,top=3'); setTimeout(function () {     popupwin.close(); }, 1000);;");
